...
agent = Agent("worker", plugins=[tool_approval, ulw], model="co/claude-opus-4-5")`}
        ...
        <section className="mb-20">
          ...
          <p className="text-gray-700 mb-4">In <code className="bg-gray-100 px-2 py-1 rounded">co ai</code>, use the mode selector to switch to ULW mode. The frontend sends:</p>
          <CodeWithResult
...
agent.input("Refactor all Python files to use type annotations")`}
        ...
        <section className="mb-20">
          ...
          <p className="text-gray-700 mb-4">Send a <code className="bg-gray-100 px-2 py-1 rounded">prompt_update</code> message to steer the agent while it works:</p>
          <CodeWithResult
            code={`{ "type": "prompt_update", "prompt": "Focus on the authentication module" }`}
            language="json"
          />
          <p className="text-gray-600 mt-4 text-sm">Injected into the system prompt before each LLM call. The agent remembers the updated goal for all subsequent turns.</p>
        </section>
